...
int good_or_bad_number(int n, char c, int* check_error_in_numbers) {
    if (c == 'm' || c == 't') {
...
void func_q(double a, double b, double c) {
    double d;
    printf("For the: (%.0lf) * x^2 + (%.0lf) * x + (%.0lf) = 0\n", a, b, c);
    if (a == 0) {
...
int main(int argc, char *argv[]) {
    ...
    char parametrs [6][3] = {"-q", "/q", "-m", "/m", "-t", "/t"};
    for (i = 0; i < 6; i += 2) {
        if (!strcmp(parametrs[i], argv[1]) || !strcmp(parametrs[i + 1], argv[1])) {
            ...
            switch (parametrs[i][1]) {
                case 'q':
                    if (argc == 5) {
                        a = conv_n_to_int(argv[2], &check_error_in_numbers);
                        ERRORS = good_or_bad_number(a, 'q', &check_error_in_numbers);
                        if (!(ERRORS)) {
                            return 0;
                        }
                        b = conv_n_to_int(argv[3], &check_error_in_numbers);
                        ERRORS = good_or_bad_number(b, 'q', &check_error_in_numbers);
                        if (!(ERRORS)) {
                            return 0;
                        }
                        c = conv_n_to_int(argv[4], &check_error_in_numbers);
                        ERRORS = good_or_bad_number(c, 'q', &check_error_in_numbers);
                        if (!(ERRORS)) {
                            return 0;
                        }
                ...
                case 'm':
                    if (argc == 4) {
                        ...
                        ERRORS = good_or_bad_number(a, 'm', &check_error_in_numbers);
                        if (!(ERRORS)) {
                            return 0;
                        }
                        k2 = conv_n_to_int(argv[3], &check_error_in_numbers);
                        ERRORS = good_or_bad_number(b, 'm', &check_error_in_numbers);
                        if (!(ERRORS)) {
                            return 0;
                        }
                ...
                case 't':
                    if (argc == 5) {
                        a = conv_n_to_int(argv[2], &check_error_in_numbers);
                        ERRORS = good_or_bad_number(a, 't', &check_error_in_numbers);
                        if (!(ERRORS)) {
                            return 0;
                        }
                        b = conv_n_to_int(argv[3], &check_error_in_numbers);
                        ERRORS = good_or_bad_number(b, 't', &check_error_in_numbers);
                        if (!(ERRORS)) {
                            return 0;
                        }
                        c = conv_n_to_int(argv[4], &check_error_in_numbers);
                        ERRORS = good_or_bad_number(c, 't', &check_error_in_numbers);
                        if (!(ERRORS)) {
                            return 0;
                        }
